...
public class SimpleInputTest {
    ...
    private void read1() {
        try (InputStream input = new ByteArrayInputStream(data1.getBytes())) {  // -> 데이터를 읽어들일 수 있음
            int read = -1;
    ...
    private void read2() {
        ...
        try (InputStream input = new ByteArrayInputStream(data2.getBytes())) {
            ...
            while ((read = input.read(buffer)) > 0) {
                ...
                System.out.printf("읽은 개수: %d, 문자열: %s%n", read, new String(buffer, 0, read)); //0부터 read 까지
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void read3() {
        ...
        try(Reader reader = new CharArrayReader(data2.toCharArray())) {
        	int read = -1;  // 더이상 읽을 것이 없음
        	while((read = reader.read(buffer))!=-1) {
        		System.out.printf("읽은 개수: %d, 문자열 : %s\n", read, String.valueOf(buffer, 0,read));
        	}
        }catch(IOException e) {
        	e.printStackTrace();
        }
